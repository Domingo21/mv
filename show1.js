function show1(genre = "Action") {
  ...
  maleLabel.classList.add("flex", "items-center", "ml-8", "mt-0");
  maleBg.classList.add("mr-2", "w-5", "h-5", "bg-purple-600");
  femaleLabel.classList.add("flex", "items-center", "ml-8", "mt-0");
...
function getRatingsNumberPerGenre(genre, occupation) {
  ...
  for (const entry of dataset) {
    ...
    if (entry.genres.includes(genre)) {
      if (entry.user.Gender === "M") {
        data[entry.rating - 1].male += 1;
      } else if (entry.user.Gender === "F") {
