function show2() {
  let width = 600;
  let height = 600;
  const margin = { top: 20, bottom: 20, left: 50, right: 25 };
  ...
  svg
    ...
    .selectAll("text") 
    .attr("transform", "rotate(-30)") // 设置旋转角度
  ...
  const color = d3.scaleSequential()
    .interpolator(d3.interpolate("#FEC7FE", "#3EEABE"))
    .domain([2.5, 5]);
  ...
  const mousemove = function (event, d) {
    tooltip
      .html(
        `Occupation: ${d[0].split("$")[0]} <br> Genre: ${d[0].split("$")[1]} <br> Average Rating: ${Math.round(d[1] * 100) / 100
        }`
      )
